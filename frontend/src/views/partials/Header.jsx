  ...
  const fetchProfile = async () => {
    try {
      ...
      if (error.response && error.response.status === 404) {
        console.warn("User profile not found. Please ensure the user exists.");
      }
    }
  };
    ...
    <header className="bg-gray-800 text-white shadow-md sticky top-0 z-50">
      <nav className="container mx-auto px-4 py-3 flex justify-between items-center">
        <Link to="/" className="flex items-center space-x-2">
          <img src="/inkspire_final_logo.png" alt="Logo" className="h-16 w-auto" />
        </Link>
            ...
            {isOpen && (
              <div className="absolute top-full mt-2 bg-gray-700 text-white rounded shadow-lg min-w-[200px]">
                <ul className="py-2">
                  ...
                  <li>
                    <Link to="/posts/" className="block px-4 py-2 hover:bg-gray-600">
                      <i className="bi bi-grid-fill mr-2"></i> Posts
                    </Link>
                  </li>
                  <li>
                    <Link to="/add-post/" className="block px-4 py-2 hover:bg-gray-600">
                      <i className="fas fa-plus-circle mr-2"></i> Add Post
                    </Link>
                  </li>
                  <li>
                    <Link to="/comments/" className="block px-4 py-2 hover:bg-gray-600">
                      <i className="bi bi-chat-left-quote-fill mr-2"></i> Comments
                    </Link>
                  </li>
                  <li>
                    <Link to="/notifications/" className="block px-4 py-2 hover:bg-gray-600">
                ...
                <img
                  src={
                    fetchError
                      ...
                      : profileData || "https://via.placeholder.com/40?text=No+Image"
                  }
                  alt="Profile"
